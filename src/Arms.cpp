// First-person ARM HIDE. The palette owns weapon placement, and the game's stock arms would
// otherwise render their canned animation while the weapon tracks the controller -- two systems
// visibly disagreeing about where the hands are. Until arm IK exists, the honest presentation is
// weapon-only: hide the FP arm meshes (all three carriers of the anim blueprint's pose -- arms,
// shield shell, shadow -- or one bone subtree, per config), re-asserted at ~8 Hz because level
// loads and weapon swaps spawn fresh visible components.

#include "Arms.hpp"

#include "BlamPalette.hpp"   // blam_palette_fp_live -- the sweep's rebuild-window gate
#include "Config.hpp"
#include "Rig.hpp"
...
// Raw read-only view of a TArray property: {data, num, max}. Layout is UE's own and stable.
struct FRawArrayRO { void* data; int32_t num; int32_t max; };

constexpr int32_t kMaxBones = 512;   // sanity bound on GetNumBones
...
int32_t call_get_bone_index(API::UObject* comp, const wchar_t* bone) {
    alignas(16) uint8_t p[RIG_PARAM_BUF] = {0};
    API::FName n = make_fname(bone);
    memcpy(p, &n, sizeof(int32_t) * 2);
    comp->call_function(L"GetBoneIndex", p);
    return *reinterpret_cast<int32_t*>(p + 8);
}
...
void call_hide_bone(API::UObject* comp, const wchar_t* bone) {
    ...
    // EVIDENCE, once per bone name: the right arm survived a hide by its VERIFIED name, so the
    // call chain itself is under suspicion. GetBoneIndex says whether this component's skeleton
    // even has the bone (INDEX_NONE = -1 means the hide call above was addressed to nobody).
    static uint32_t s_said = 0;
    if (s_said < 8) {
        ++s_said;
        API::get()->log_info("[Halo-CampE-UEVR] ARMHIDE bone '%ls' on %ls: index %d (fname %d)",
                             bone, class_name_of(comp).c_str(),
                             call_get_bone_index(comp, bone), n.comparison_index);
    }
}
...
int sweep_fp_meshes(bool hide) {
    ...
    // COMMA-SEPARATED bone list. The palette-weapon presentation hides BOTH arms by bone
    // ("Shoulder_L,Shoulder_R" -- each shoulder roots its whole arm subtree) while the weapon
    // branch stays visible: hiding the whole mesh (mode 2) takes the palette-driven gun with it,
    // leaving the camera-glued weapon actor as the only gun on screen -- which orbits the view.
    std::wstring bones[8];
    int nbones = 0;
    {
        ...
        while (*s2 != 0 && nbones < 8) {
            ...
            while (*e != 0 && *e != ',') ++e;
            std::string one(s2, e);
            // Trim EVERYTHING at or below space -- this file is CRLF, and a bone name carrying a
            // trailing '' resolves (FName Add-mode creates it!) and hides nothing. The log line
            // that finally caught it printed the name and then truncated mid-format on the CR.
    ...
    // Mode 3 (weapon-only): the FP pawn is MODULAR -- armor pieces are separate skeletal-mesh
    // components of the same classes, and an arm bone name that exists on the main mesh is a
    // silent no-op on them (field-observed: arms gone, armor still floating). The component the
    // rig tracks is the one carrying the weapon bones, so: whole-hide every swept component
    // EXCEPT the tracked one, and bone-hide the arm list on it. The weapon branch keeps drawing.
...
void arms_hide_update() {
    // BONEDUMP one-shot: arm on the config edge, fire once the rig resolves. Exists to READ bone
    // names off this build's FP skeleton instead of guessing them -- the right-arm root turned
    // out not to be the assumed name, and the fix starts with the list.
    {
        static bool s_bd_prev = false, s_bd_armed = false;
        const bool bd = g_cfg.bone_dump;
        if (bd && !s_bd_prev) s_bd_armed = true;
        s_bd_prev = bd;
        if (s_bd_armed) {
            if (auto* rig = rig_tracked_component()) { s_bd_armed = false; arms_dump_skeleton(rig); }
        }
    }
    ...
    // HOLD OFF WHILE THE FP BUILD IS DARK, AND FOR TWO SECONDS AFTER IT RETURNS. Bisected
    // 2026-08-26: with the sweep running, every respawn froze the palette-driven PrimaryWeapon
    // socket at the stock pose ~1 s in -- the hide lands on the freshly rebuilt components
    // mid-initialization and the FP palette sync never binds, so the weapon actor rides the
    // camera-parented mesh at its stock socket ("attached to the rig"). The same death with
    // armhide=0 tracks perfectly, and the pre-death sweep on settled components never hurt
    // anything.
    //
