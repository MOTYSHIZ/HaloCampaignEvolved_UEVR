...
struct Config {
    ...
    //
    // This is a STOPGAP with a known end date: it exists because tracked player arms are not part of this build. The
    ...
    // What happens to the IN-SCENE widget reticule once the layer has been live at least once:
    //   0 leave it drawing (two reticules), 1 alpha to zero (keeps the widget rendering so the
    //   layer's copied art keeps animating), 2 shrink by xr_layer_hide_scale,
    //   3 bVisibleInSceneCaptureOnly, 4 mode 3 plus bOwnerNoSee.
    ...
    // Read by XrLayer.cpp / ScopeMask.cpp for slots this tree never publishes (grab guide, scope
    // pane). Present so the module compiles unchanged; no parse keys, nothing else reads them.
    ...
    //
    // create_projectile is shared by EVERYTHING that fires -- NPC, teammates in co-op, scenery. An
    ...
    // owner/shooter field. Needed because call-site filtering cannot separate player from NPC.
    int   blam_dump = 0;
    ...
    // comfort tuning like cutscenesize, live (~2 s).
    ...
    //                    CANON -15 (2026-09-14, user's headset call for v0.4.5): 0 read as high.
