...
void on_post_render_dx12(void* cmd_list_v, void* rt_resource_v, void* rtv_v) {
...
#if HALO_VR_DEV
    const bool dump_wanted = (g_cfg.cutscene_dump != 0);
#else
    const bool dump_wanted = false;
#endif
    if (g_dead || (!g_cfg.scope_blit && !cine && !dump_wanted)) return;
...
#if HALO_VR_DEV
    // ---- ONE-SHOT EYE DUMP (cutscenedump). Reads back the WHOLE side-by-side scene target ----
    //
    // Why a picture and not another log line: cutscenemono modes 1-4 (2026-09-08) proved the
    // movie lives in the projection images and that both eyes already carry the same thing, so
    // whatever "doubled" means is a property of ONE eye image, and no counter can describe it.
    //
    ...
    //     There is no fence we can wait on from here, so the buffer is read TWO callbacks later,
    //     by which point that list has long since executed and presented.
    //   * The scene target is assumed to be in RENDER_TARGET here (UEVR's own eye copy restores
    //     it to that state), and it is transitioned to COPY_SOURCE and back around the copy. If
    //     the assumption is wrong the debug layer objects and the pixels are undefined -- which
    //     is recoverable and visible, as opposed to skipping the barrier and hoping.
    // Written as a 32bpp top-down BMP straight from the BGRA8 rows; no image library.
    {
        ...
        if (want != 0 && s_seen == 0 && s_state == 0) {
            s_seen = want;
            ID3D12Device* dev = nullptr;
            if (SUCCEEDED(src->GetDevice(__uuidof(ID3D12Device), (void**)&dev)) && dev != nullptr) {
        ...
        } else if (s_state == 1 && ++s_wait >= 2) {
            char dir[MAX_PATH] = {0};
            const DWORD n = GetEnvironmentVariableA("APPDATA", dir, MAX_PATH);
            char path[MAX_PATH] = {0};
            if (n > 0 && n < MAX_PATH) {
                sprintf_s(path, MAX_PATH, "%s\\UnrealVRMod\\HaloCampaignEvolved\\data\\eyedump-%lu.bmp",
                          dir, (unsigned long)GetTickCount());
            }
            void* p = nullptr;
            const D3D12_RANGE rr{ 0, (SIZE_T)s_rb_size };
            FILE* fo = nullptr;
            if (path[0] != 0 && SUCCEEDED(s_rb->Map(0, &rr, &p)) && p != nullptr) {
                ...
                API::get()->log_info("[Halo-CampE-UEVR] EYEDUMP: Map failed or APPDATA unreadable");
            }
            s_rb->Release(); s_rb = nullptr;
            s_state = 2;
        }
    }
    if (!g_cfg.scope_blit && !cine) return;   // dump-only frame: nothing to draw
#endif
...
void scope_blit_register() {
    // CALLED ONCE FROM on_initialize, NEVER FROM A TICK. See the header for the deadlock this
    // shape exists to avoid; in short, add_on_post_render_vr_framework_dx12() takes a unique_lock
    // on the same shared_mutex that on_pre_engine_tick's dispatch is already holding shared on
    // this very thread, and std::shared_mutex is neither recursive nor upgradeable.
    if (g_registered || g_dead) return;

    auto* param = API::get()->param();
    if (param == nullptr || param->callbacks == nullptr) {
        // At init this is fatal rather than transient: the param IS what UEVR just handed us, so a
        // null here means the surface is not what this build expects, and no later tick will fix
        // it. die() disables the feature loudly instead of leaving it silently inert.
        die("no plugin param/callbacks at initialize -- cannot register the render callback");
        return;
    }
    ...
    // UNCONDITIONAL registration, so this line says the callback is live but idle -- it is not a
    // claim that either lane is switched on. Both are read per frame inside the callback.
    API::get()->log_info("[Halo-CampE-UEVR] SCOPEBLIT: render callback registered at init "
                         "(scopeblit=%d cutsceneblit=%d -- inert until one of them is set)",
