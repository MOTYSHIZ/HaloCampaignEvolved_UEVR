...
void reticle_rescan(uint32_t tick) {
    ...
    // live), so it is widened once per sweep rather than per object. The default lives HERE,
    // not in the struct initializer: a char-array string default on the global g_cfg does not
    // survive MSVC's constant-initialization (see the note on nav_class in Config.hpp).
    g_nav_count = 0;
