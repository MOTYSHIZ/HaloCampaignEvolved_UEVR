...
#include "core/PalettePose.hpp"     // the on-foot reticule's STOMPLOG marks
#include "core/ReticuleDepth.hpp"   // the on-foot reticule's trace depth publish
#include "core/UnitState.hpp"
#include "core/registry/Features.hpp"   // FEATURE REGISTRY hook: startup resolved-state log
    ...
    if (g_cfg.aim_reticule) {
        ...
        if (have_origin) {
            ...
            // Only park_marker belongs under that condition.
            {
                ...
                halo::palette_pose_mark(40, origin.x, origin.y, origin.z, d);
                halo::palette_pose_mark(41, target.x, target.y, target.z, (float)features_reticule_render_publish_mode());
                halo::palette_pose_mark(42, r_yaw, r_pitch, (float)ray_yaw, (float)ray_pitch);
                halo::palette_pose_mark(43, g_view_pos_x.load(), g_view_pos_y.load(), g_view_pos_z.load(), (float)tick);
...
void update() {
    ...
    features_game_tick_after_rig_driver(aim_yaw, aim_pitch, tick);
...
public:
    ...
    void on_post_calculate_stereo_view_offset(UEVR_StereoRenderingDeviceHandle, int index, float,
        ...
        // COMPOSITOR RETICULE, PLACED AT RENDER RATE. The tick chose the world point; this decides
