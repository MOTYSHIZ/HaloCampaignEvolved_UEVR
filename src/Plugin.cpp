...
public:
    ...
    void on_pre_calculate_stereo_view_offset(UEVR_StereoRenderingDeviceHandle, int index, float,
        ...
        if (is_double) {
            ...
            if (!g_lock_primed.load()) {   // first frame: capture where the view currently is
                g_locked_view_yaw = (float)r->yaw;
                g_lock_primed = true;
            } else {
                r->yaw = (double)locked;
            }
            ...
            if (!g_lock_primed.load()) {
                g_locked_view_yaw = rotation->yaw;
                g_lock_primed = true;
            } else {
                rotation->yaw = locked;
            }
