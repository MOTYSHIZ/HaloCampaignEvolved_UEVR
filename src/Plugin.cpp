...
// CAMERA BOB: the fast part of (camera - pawn root), world cm. The palette's world hand
// subtracts it so view and gun stay consistent. The bob-cancel PUBLISHER is not ported
// yet, so these hold zero -- exactly the bobcancel=0 behaviour of the source tree.
...
void update() {
    ...
    // The gesture detectors (melee swing, the reload state machine) run on the same poses the
    // pose publisher just fetched. BEFORE two_hand_update: both read the same grip, and the
    // two-hand latch must see the state the gestures have already settled for this tick.
    gesture_update(g_last_dt.load());
    two_hand_update(g_last_dt.load());
    // Holsters and grenade pouches: after the gestures (same poses, same grip). Never in menus --
    // an involuntary transition hands the weapon back and drops any armed grenade.
    if (g_in_menu.load()) holster_reset(); else holster_update(g_last_dt.load());
    ...
    const uint32_t trace_every = halo::blam_palette_freeze_active() ? 1u : 45u;
    if (g_cfg.palette_weapon && (tick % trace_every) == 0u) {
        ...
        if (g_cfg.palette_weapon_log) API::get()->log_info(
            "[Halo-CampE-UEVR] TRACE-JUDGE hand_world=(%.0f %.0f %.0f)cm gun_rel_eye=(%.0f %.0f %.0f)cm "
            "ERR=(%.0f %.0f %.0f)cm |%.0f|cm  ERR_in_cam=(%.0f %.0f %.0f)cm  lock_gap=%.0f  "
            "cam_p=%.0f head_p=%.0f dpitch=%.0f  reach=%.0fcm  wpn=%s",
            ...
            wrap180((float)cy - view_yaw), (float)cp, hp, wrap180((float)cp - hp),
    ...
    if (g_cfg.palette_weapon) {
        ...
        if (comp != nullptr && g_rig_parent != nullptr &&
            ...
            if (call_ret_vec3(comp, L"K2_GetComponentRotation", &crot) &&
                ...
                const Quat cam  = rotator_to_quat((float)cp, (float)cy, 0.0f);
    ...
    if (g_cfg.palette_weapon && g_cfg.palette_probe) {
        static Vec3     s_sum[6] = {};
    ...
    // hidden binding around in the meantime.
    {
        ...
        // solve -- and differs only in where the answer is stored. Polled here so both
        // edge-detections live on one thread.
        wpn_calib_poll();
        const bool held = (key_focus && (g_cfg.calib_key != 0) &&
                           ((GetAsyncKeyState(g_cfg.calib_key) & 0x8000) != 0)) ||
                          (menu_mode == 1 && !menu_lt);
        const bool was  = g_calib_held.exchange(held || wpn_calib_held());
    ...
    // controller pointer and the rig pointer. Nothing below is required.
    {
        ...
        // (docs\VEHICLE_CAMERA_FINDINGS.md), and one logged vehicle entry settles it.
        {
            ...
            const bool palette_fp = g_cfg.palette_weapon && blam_palette_fp_live();
    ...
    if (g_cfg.rig_enabled || g_cfg.palette_weapon) {
...
public:
    ...
    void on_xinput_get_state(uint32_t* retval, uint32_t user_index, XINPUT_STATE* state) override {
        ...
        // Publish the RAW buttons before any remapping below: consumers (the two-hand latch)
        // must react to what the player physically pressed, not what a remap produced.
        halo::g_pad_buttons.store(state->Gamepad.wButtons, std::memory_order_relaxed);
        // Tap/hold tracking at POLL rate, not tick rate -- a 250 ms threshold judged at ~32 Hz
        // quantises to 31 ms steps and feels arbitrary in the hand.
        ...
        // ---- HOLSTERS: physical swap/switch/throw buttons swallowed first (the gestures own
        // those actions now -- holsterstealbtns), THEN the synthetic presses injected, so the
        // gesture-driven press is the only path to the game. Menus and stick mode keep the
        // buttons: Y navigates UI, and stick mode has no holsters.
        // Never while MOUNTED: holsters cannot operate from a vehicle seat, and the stolen
        // buttons have day jobs there (2026-08-20: stealing them in a hog read as "controllers
        // stopped working" -- the actions the seat needed were being eaten).
        if (g_cfg.holster_steal_buttons != 0 && g_cfg.holster_enabled
            && !g_unit_mounted.load(std::memory_order_relaxed)
            && !g_in_menu.load(std::memory_order_relaxed) && !g_stick_mode.load(std::memory_order_relaxed)) {
            // The GRIP is swallowed too while the two-hand hold can use it: the same physical
            // grip is the game's grenade button (LB), so without this a left grip latched the
            // two-hand hold AND lobbed a game grenade in the same squeeze -- field-observed as
            // "left grip throws grenades" plus "the aim ray shoots off somewhere else" (the
            // latch blending aim toward the support hand). The gestures own the grip on foot.
            const WORD grip_steal = (g_cfg.two_hand && g_cfg.reload_grip_mask != 0)
                                  ? (WORD)g_cfg.reload_grip_mask : (WORD)0;
            const WORD steal = (WORD)((WORD)g_cfg.holster_swap_mask | (WORD)g_cfg.holster_throw_mask
                                    | (WORD)g_cfg.holster_gswitch_mask | grip_steal);
            const WORD before = state->Gamepad.wButtons;
            state->Gamepad.wButtons &= (WORD)~steal;
            // 2026-08-20: this mask read as "ALL input dead" twice, which three bits cannot do --
            // so log what it ACTUALLY holds and removes rather than trusting the cfg values.
            // Once at first run, then on every poll where it changes a bit.
            if (g_cfg.map_btn_log) {
                static bool said = false;
                if (!said) {
                    said = true;
                    API::get()->log_info("[Halo-CampE-UEVR] STEAL: masks swap=0x%04X throw=0x%04X gswitch=0x%04X -> steal=0x%04X keep=0x%04X",
                                         (unsigned)(WORD)g_cfg.holster_swap_mask, (unsigned)(WORD)g_cfg.holster_throw_mask,
                                         (unsigned)(WORD)g_cfg.holster_gswitch_mask, (unsigned)steal, (unsigned)(WORD)~steal);
                }
                if (before != state->Gamepad.wButtons) {
                    API::get()->log_info("[Halo-CampE-UEVR] STEAL: 0x%04X -> 0x%04X (removed 0x%04X)",
        ...
        if (g_cfg.holster_swap_mask != 0 && holster_swap_press_active())  state->Gamepad.wButtons |= (WORD)g_cfg.holster_swap_mask;
        if (g_cfg.holster_throw_mask != 0 && holster_throw_press_active()) state->Gamepad.wButtons |= (WORD)g_cfg.holster_throw_mask;
        if (g_cfg.holster_gswitch_mask != 0 && holster_gswitch_press_active()) state->Gamepad.wButtons |= (WORD)g_cfg.holster_gswitch_mask;
        ...
        // ...and put the reload press back only when WE decide the magazine is seated. After the
        // strip, so the strip cannot eat our own synthetic press.
        if (g_cfg.reload_mask != 0 && reload_press_active()) {
            state->Gamepad.wButtons |= (WORD)g_cfg.reload_mask;
        }


        // ---- FIRE SUPPRESSION. While the weapon is stowed or a grenade is in the hand, the
        // trigger is swallowed: the gun is hidden in both states, and an invisible weapon that
        // still shoots is worse than either state alone.
        if (holster_fire_suppressed()) {
            state->Gamepad.bRightTrigger = 0;
        }
