...
const char* arm_driver_name(ArmDriverMode mode) {
    if (const char* name = features_arm_driver_name((int)mode)) return name;
    switch (mode) {
...
ArmDriverMode arm_driver_mode() {
    ...
    if (const int m = features_arm_driver_mode_wanted(); m > 0) return (ArmDriverMode)m;
...
ArmDriverMode effective_mode() {
    ...
    if (features_arm_driver_mode_unavailable((int)wanted)) return ArmDriverMode::UeRig;
    return wanted;
}
...
void arm_driver_release_all(const char* why) {
    ...
    arms_release_hide();
    palettearm_release();
    features_arm_driver_release_all(why);
    if (why != nullptr) {
...
void arm_driver_arbitrate() {
    ...
    if (features_arm_driver_key_changed() || g_cfg.arm_driver != s_last_cfg) {
    ...
    if (!s_announced) {
        ...
        s_announced = true;
        s_active = wanted;
        features_arm_driver_active((int)wanted, false);
        return;
    }

    if (wanted == s_active) return features_arm_driver_steady((int)s_active);
    ...
    s_active = wanted;
    features_arm_driver_active((int)wanted, true);
}
