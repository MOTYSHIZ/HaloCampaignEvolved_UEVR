...
void weapon_object_nodes_probe() {
    ...
    const bool node_work = g_cfg.wpn_node_dump || g_cfg.wpn_node_poke >= 0 || (g_cfg.slide_vr && g_cfg.slide_part_hide != 7);
    ...
    if (s_off >= 0 && g_cfg.slide_vr && g_cfg.slide_node >= 0 && g_cfg.slide_node < s_count) {
...
void blam_capture_hook_tick() {
    const bool want = g_cfg.slide_vr && g_cfg.slide_hook;
    if (!want) {
...
// Resolve the held weapon's Blam object from a sim-thread call site OTHER than the builder hook, so
// the manual reload keeps its object read under the author's arm drivers (the builder hook only
// exists in mode 3). Called from BlamDrive's orientation hook, rate-limited there.
void blam_weapon_object_probe_offhook() {
    if (palette_weapon_mode()) return;   // mode 3: the builder hook already runs it
    weapon_object_nodes_probe();
}
