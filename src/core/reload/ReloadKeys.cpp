...
bool reload_engine_parse_key(const char* key, const char* val, double v) {
    ...
    if (_stricmp(key, "reloadmagpick")  == 0) { g_cfg.reload_mag_pick = (int)clampf((float)v, 0.0f, 3.0f); return true; }
    if (_stricmp(key, "zonesnap")       == 0) { g_cfg.zone_snap = (int)clampf((float)v, 0.0f, 3.0f); return true; }
