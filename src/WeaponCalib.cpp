...
// halo_vr.cfg, and "AssaultRifle" is that. Strip the known prefix and suffix; if the shape is
...
void wpnfix_set(const std::string& key, const float q[4], const float t[3]) {
    ...
    int slot = -1;
    for (int i = 0; i < g_cfg.wpnfix_count; ++i) {
        if (g_cfg.wpnfix[i].match[0] != 0 && key.find(g_cfg.wpnfix[i].match) != std::string::npos) { slot = i; break; }
    }
    if (slot < 0) {
        if (g_cfg.wpnfix_count >= kMaxWeaponAdjust) return;
...
void wpn_calib_write_file() {
    ...
    if (g_wpn_calib_path[0] != 0 && fopen_s(&f, g_wpn_calib_path, "wb") == 0 && f != nullptr) {
        ...
        if (g_cfg.wpnfix_count > 0) {
            fprintf(f, "\r\n# PALETTE per-weapon rigid delta, composed onto gripfix (Home key freeze-and-align).\r\n"
                       "# wpnfix=<match>,qx,qy,qz,qw,tx,ty,tz  -- quaternion then METRES, aim-pose frame.\r\n");
            for (int i = 0; i < g_cfg.wpnfix_count; ++i) {
