...
#include "features/vehcam/BlamDrive_seat_atomics.inl"   // fork feature: vehcam (seat publish evidence)
...
static void drive_angles_impl(bool off_thread) {
    ...
    if (g_stick_mode_active.load(std::memory_order_relaxed)) {
        #include "features/vehcam/BlamDrive_seat_stickmode.inl"   // fork feature: vehcam (stick-mode seat publish)
        return;
    }
